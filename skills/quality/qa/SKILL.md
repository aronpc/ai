...
|-------|-------------------|
| `@laravel/testing` | Para execução de testes automatizados |
| `@gh/pr-review` | Para review de PR com foco em qualidade |
| `@devops/mcp` | Para validação visual da aplicação |
| `@dev/coder` | Para corrigir issues encontradas |
| `@plan/sprint` | Para atualizar status após validação |
